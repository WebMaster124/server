...
class CheckoutController extends Controller{
        ...
        try {
            ...
            if(Hash::check($auth,$authDB->auth)) {
                ...
                if ($r->cart == 'true') {
                    ...
                    $product_info=$qry->get(['p.id as product_id','p.payment_opt','p.quantity as product_quantity', 'p.title', 'p.photos', 'p.discount', 'p.price', 'p.delivery_days', 'p.user_id as seller_id',
                    ...
                    $product_info = DB::table('product')->where('id', $r->product_id)
                        ->first(['id', 'title', 'photos', 'discount', 'price','payment_opt' ,'delivery_days', 'user_id','quantity as product_quantity']);
                    ...
                    $cart=false;
                    $seller_info=DB::table('users')->where('id',$product_info->user_id)->first(['display_name','payment_email','user_status']);
    ...
    function placeOrder(Request $r){
        try{
            ...
            $this->reply['place_order']=['status'=>'fail','msg'=>$e->getMessage()];
            return response()->json($this->reply);
            DB::rollBack();
        }
    }
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                $orders=DB::table('order as o')
                    ...
                    ->get([
                        'us.display_name',
                        'p.title','p.photos','p.price','p.shipment_type','p.discount','p.product_sku',
                        'o.specification','o.status','o.seller_id','o.quantity as order_quantity','o.order_id','o.order_id','o.order_id','o.order_id','o.product_id','o.coupon_code','o.order_price','o.created_date',
