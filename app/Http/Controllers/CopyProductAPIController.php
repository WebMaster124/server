...
class ProductAPIController extends Controller
{
        ...
        try {
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                $product=$qry->select('p.id as product_id','p.product_sku','p.title','p.subtitle','p.category_id','p.photos','p.user_id as seller_id','p.approve_date',
                    ...
                    'p.shipment_type','p.delivery_days','p.paypal_email','p.updated_date','p.return_policy','p.deals_days','p.season_deal','p.min_price',
   ...
   function editProduct(Request $r){
       try{
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                $productOldImages=DB::table('product')->where('id',$r->id)->first(['photos','daily_deal','duration']);
                ...
                $product=['user_id'=>$r->user_id,'title'=>$r->title,'subtitle'=>$r->subtitle,
                    'category_id'=>$r->category_id,'secondary_cat_ids'=>$secondary_cat_ids,'photos'=>$imageName,'isbn'=>$r->isbn,'settype_id'=>$r->settype_id,'brand_id'=>$r->brand_id,'p_condition'=>$r->p_condition,
                    'condition_description'=>$r->condition_description,'specification'=>$specification,'description'=>$r->description,
                    'quantity'=>$r->quantity,'price'=>$r->price,'payment_opt'=>$payment_opt,'return_opt'=>$r->return_opt,
                    'return_policy'=>$r->return_policy,'discount'=>$discount,'renew'=>$renew ,
                    'selling_type'=>$r->selling_type,'product_address'=>$r->product_address,'warrenty'=>$r->warrenty,'season_deal'=>$r->season_deal,
                    'ship_to'=>$r->ship_to,'daily_deal'=>$r->daily_deal,'deals_days'=>$r->deals_days,'shipment_type'=>$shipment_type,'delivery_days'=>$r->delivery_days,
                ...
                if($update){
                    ...
                    if($r->from_product=="ADD_PRODUCT"){
                        DB::table('product')->where(['id'=>$r->id,'user_id'=>$r->user_id])->update(['renew_duration' =>$r->duration]);
   ...
   function recentlyViewedItems(Request $r){
        ...
        //->where('r.created_date', '>=', Carbon::now()->subDays(30))
                ...
                $products=$qry->get(['p.title', 'p.photos','p.selling_type','p.min_price','p.price', 'p.id','p.duration','p.renew_duration','p.created_date','p.discount','p.quantity','p.approve_date']);
                foreach ($products as $product) {
        ...
        try{
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                $products=DB::table('recently_view as r')
                    ->join('product as p','p.id','=','r.product_id')
                    ->where('p.status','=','1')
                    ->select("r.product_id",'p.selling_type','p.min_price','p.title','p.photos','p.price','p.duration','p.renew_duration','p.created_date','p.approve_date','p.discount','p.quantity',DB::raw("COUNT(tbl_r.product_id) as count"))
   ...
   function dailyDeals(Request $r){
        try{
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                $products=$qry->get(['title','selling_type','min_price','discount','photos','price','id','duration','renew_duration','created_date','discount','quantity','approve_date']);
                foreach ($products as $product){
   ...
   function viewProductDraft(Request $r){
        try{
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                $this->reply['draft_product']=['status'=>'success','data'=>$products];
            }else{
                $this->reply['Auth']=['status'=>'fail','msg'=>'Authentication Fail.'];
            }
            return response()->json($this->reply);
   ...
   function categoryProductList(Request $r){
        try{
            ...
            if(Hash::check($auth,$authDB->auth)) {
                ...
                $qry = DB::table('product as p')->where('p.status', '1');
                ...
                if($r->season_deal)
                    $qry = $qry->where('p.season_deal','Yes');
   ...
   function listCount(Request $r){
        try{
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                //ACTIVE PRODUCT
                $data['active']=DB::table('product')->where(['user_id'=>$user_id])->where(function($q){
                    $q->orwhere('status','1');
                    $q->orwhere('status','3');})
                   ->count();
                //DRAFT PRODUCT
       ...
       try{
            ...
            if(Hash::check($auth,$authDB->auth)){
                ...
                foreach ($save_search as $search){
                    ...
                    $products=$qry->get(['id','title','photos','selling_type','min_price','product_sku','discount','quantity','price','duration','renew_duration','created_date','approve_date']);
                    foreach ($products as $product){
                        $product->photos=unserialize($product->photos);
   ...
   function viewBlocks(Request $r){
       try{
           ...
           if(Hash::check($auth,$authDB->auth)){
              ...
              $this->reply['blocks']=['season_deal'=>$season,'banners'=>$banner ,'latest_trending'=>$trandingProducts,'daily_deals'=>$dailyProducts,'recently_viewed'=>$recentlyViewProducts,'recommended'=>$recommendedProductsInfo,'new_products'=>$newProducts,'block'=>$block];
           }else
               $this->reply['Auth']=['status'=>'fail','msg'=>'Authentication Fail'];
   ...
   function newProducts(Request $r){
       try {
           ...
           if (Hash::check($auth, $authDB->auth)) {
               ...
               $newProducts = $qry->get(['title', 'discount','min_price','selling_type', 'photos', 'price', 'id', 'duration', 'renew_duration', 'created_date', 'discount', 'quantity', 'approve_date']);
               foreach ($newProducts as $product) {
                   $product_images = unserialize($product->photos);
                   $product->photos = $product_images;
                    if($product->selling_type=="Auction style")
    ...
    function underFiftyProduct(Request $r){
        try {
            ...
            if (Hash::check($auth, $authDB->auth)) {
                ...
                $products=$qry->get(['title', 'photos', 'price', 'id','duration','renew_duration','created_date','discount','quantity','approve_date','product_address','selling_type']);
                foreach ($products as $product) {
                    $product_images = unserialize($product->photos);
                    $product->photos = $product_images;
