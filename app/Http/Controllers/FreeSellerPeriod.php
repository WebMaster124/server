...
class FreeSellerPeriod extends Controller
{
    function viewSellers(){
        ...
        return view('admin/add_free_seller_period')->with('users',$seller);
    }
...
//            exit;
            foreach ($r->seller_id as $id){
                $sets = ['seller_id' => $id, 'start_date' => $start_date, 'end_date' => $end_date, 'checklist_id' => $checklist];
    ...
    function viewFreeSellerPeriod(){
        try{
            ...
            $seller=DB::table('free_seller_period as f')
                ...
                ->get(['f.id','f.start_date','f.end_date','f.checklist_id','u.first_name','u.last_name','u.display_name']);
    ...
    function editFreeSellerPeriod($id){
        try{
            ...
            $seller=DB::table('free_seller_period as f')
                ->join('users as u','u.id','=','f.seller_id')
                ->where('f.id',$id)
                ->first(['f.id','f.start_date','f.end_date','f.checklist_id','u.first_name','u.last_name','u.display_name']);
            if($seller)
    ...
    function editFreeSellerPeriodData(Request $r){
        try {
            ...
            $end_date = $r->end_date;
            $checklist = implode(',', $r->checklist);
            $sets = ['start_date' => $start_date, 'end_date' => $end_date, 'checklist_id' => $checklist,'updated_date'=>date('Y-m-d h:i:s')];
