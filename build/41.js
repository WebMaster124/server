...
var ConfirmAccountPage = /** @class */ (function () {
    ...
    ConfirmAccountPage.prototype.resendMail = function () {
   
         var user_data = {
            id: this.user_id,
            email: this.emailID
        };
        this.infoService.callHttpFunction_post(__WEBPACK_IMPORTED_MODULE_4__environments_environment__["a" /* environment */].serverUrl + 'api/user/resend-email', user_data, function (response) {
            
//           alert(response.resend_otp);
            
            if (response.resend_otp.status == "success") {
//                _this.navCtrl.push('ConfirmAccountPage');
                alert(response.resend_otp.msg);
            }
            else {
