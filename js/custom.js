...
jQuery(document).ready(function () {
    ...
    $('.logout-nav li>a').on('click', function (e) {
        // save the latest tab; use cookies if you like 'em better:
        localStorage.removeItem('lastTab', $(this).attr('href'));
    });*/

    localStorage.setItem('lastTab', $(location).attr("href"));

    // go to the latest tab, if it exists:
    var lastTab = localStorage.getItem('lastTab');
    if (lastTab) {
        if(lastTab != 'http://192.168.1.123/dibdaa/public/index.php/dashboard'){
            $('.nav-pills li').removeClass('active');
            $('[href="' + lastTab + '"]').parent().parent().parent().addClass('nav-active');
            $('[href="' + lastTab + '"]').parent().parent().css('display', 'block');
