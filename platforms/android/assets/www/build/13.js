    ...
    SellerInvoiceDetailsPage.prototype.paytoAdmin = function () {
        ...
        this.infoService.callHttpFunction_post(__WEBPACK_IMPORTED_MODULE_3__environments_environment__["a" /* environment */].serverUrl + 'api/pay/seller-paypal-payment', passdatasellerinvoice, function (response) {
            if (response.responseEnvelope.ack == "Success") {
                var getData_decode = _this.navParams.data.id;
                var convert_decode = atob(getData_decode);
                var sellerBusiness_name = JSON.parse(convert_decode);
                var storePaymentDetails = {
                    ...
                    month: sellerBusiness_name.month,
                    year: sellerBusiness_name.year,
                    invoice_id: _this.showInvoicedata.date.invoice_id
                };
                ...
                if (_this.plt.is('core') == false && _this.plt.is('mobileweb') == false) {
                    var target = "_blank";
                    var options = {
                        location: 'yes',
                        hidden: 'no',
                    ...
                    var browser_1 = _this.iab.create("https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_ap-payment&paykey=" + response.payKey, target, options);
                    browser_1.on('loadstop').subscribe(function (event) {
                        if (event.url == "http://192.168.1.113:8100/#/paymentsuccess/UqGVdpWVM0Xcjsw05X8uv4KdAaoQVw77" || event.url == "https://www.dibdaa.com/#/paymentsuccess/UqGVdpWVM0Xcjsw05X8uv4KdAaoQVw77" || event.url == "https://www.techcronus.com/staging/Dibdaa/www/#/paymentsuccess/UqGVdpWVM0Xcjsw05X8uv4KdAaoQVw77") {
                            browser_1.close();
                            _this.navCtrl.push('SellerpaymentSuccessPage', {
