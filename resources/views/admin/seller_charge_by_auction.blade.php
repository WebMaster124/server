...
<!-- headerbar -->
    <div class="pageheader">
        <h2> Fees Charged to Sellers by Auction Type  <button class="btn btn-primary" onclick="history.go(-1);"><i style="font-size:10px" class="fa">&#xf060;</i> Go Back </button></h2>
    </div>
<?php
$year = Request::segment(4);
$month = Request::segment(5);
$startdate = Request::segment(6);
$enddate = Request::segment(7);
	...
	<input type="hidden" name="year" id="year" value="<?php echo ($year != "" && $year != 0)? $year:'-'; ?>">
	<input type="hidden" name="startdate" id="startdate" value="<?php echo ($startdate != "")? $startdate:'-'; ?>">
	<input type="hidden" name="enddate" id="enddate" value="<?php echo ($enddate != "")? $enddate:'-'; ?>">
            ...
            <div class="panel-body">
                ...
                <div class="table-responsive">
                    <table class="table" id="Export">
                        ...
                        <tbody>
                        <?php foreach ($fees_auction_type as $product){ ?>
                        <tr>
                            ...
                            <td><?php
                                    $fees=0;
                                         $f=unserialize($product->fees_details);
                                        foreach ($f as $key=>$value){
                                            if($key=='featured_fees'){
                                               echo ucfirst(str_replace('_',' ',$key)) .' : '. $value[1] .'<br>';
                                                $fees=$fees+$value[1];
                                            }
                                             ...
                                             echo ucfirst(str_replace('_',' ',$key)) .' : '. $value[0] .'<br>';
                                             $fees=$fees+$value[0];
                                         }
                                        }
                                    ?></td>
		...
		buttons: [
            {
                ...
                buttons: [
							{
							extend: 'excelHtml5',
							title: 'Data Export - Fees Charged to Sellers by Auction Type'
						    },
							{
							extend: 'csvHtml5',
							title: 'Data Export - Fees Charged to Sellers by Auction Type'
							...
							filename:'Data Export - Fees Charged to Sellers by Auction Type',
							title: null,
                            customize: function ( doc ) {
								...
								doc.content.splice( 0, 0, {
									...
									columns: [
											{
												text: 'Data Export - Fees Charged to Sellers by Auction Type',bold: true
											}
										]	
								});
                                doc.content.splice( 0, 0, {
